...
pos_week_pl <- ggplot(mapping = aes(x = week, y = n)) +
  geom_bar(data = df_pos %>% mutate(class = paste("Class", class)), mapping = aes(fill = variable),
           stat = "identity") +
  geom_text(data = subset(df_pos, n > 0) %>% mutate(class = paste("Class", class)), mapping = aes(label = n), 
  ...
  labs(y = "Number of positive samples", x = "Study week", fill = "Pathogen", title = "c") +
  theme_custom() +
...
pos_cond_pl <- ggplot(mapping = aes(x = aircleaner, y = n, fill = variable)) +
  ...
  labs(y = "Number of positive samples", x = "Air cleaner", fill = "Pathogen", title = "b") +
  theme_custom() +
  theme(panel.grid = element_blank(),
...
pos_tot_pl <- df_pos_tot %>%
  ...
  labs(y = "Number of positive samples", x = "Pathogen", fill = "Pathogen", title = "a") +
  theme_custom() +
  theme(panel.grid = element_blank(),
        legend.position = "bottom") +
  guides(fill = guide_legend(nrow = 1))
