...
pos_cond_pl <- ggplot(mapping = aes(x = aircleaner, y = n, fill = aircleaner)) +
  geom_bar(data = df_pos_stud, stat = "identity", position = "stack", color = "black") +
...
pos_tot_pl <- ggplot(mapping = aes(y = n, x = 1, fill = variable)) +
  geom_bar(data = df_pos_tot, stat = "identity", position = position_stack(reverse = T)) +
  geom_text(data = subset(df_pos_tot, n != 0), mapping = aes(label = n), 
