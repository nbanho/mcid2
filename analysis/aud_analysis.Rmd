...
coughs_pl <- coughs %>%
  ...
  ggplot(aes(x = aircleaner, y = coughs_per_min, fill = aircleaner)) +
  geom_bar_pattern(mapping = aes(pattern = class),
                   stat = "identity", position = position_dodge(width = .66), color = "black", pattern_fill = "white", pattern_angle = 45,
                   pattern_density = 0.25, pattern_spacing = 0.05, width = .5) +
  geom_text(data = coughs %>% group_by(aircleaner) %>% summarize(coughs_per_min = sum(n_coughs) / sum(TT)) %>% ungroup() %>%
              mutate(y = 4.5),
            mapping = aes(y = y, label = paste("Mean =", round(coughs_per_min, 1))),
            size = 8 / cm(1), vjust = -1) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.1)), breaks = seq(0, 10, 2)) +
