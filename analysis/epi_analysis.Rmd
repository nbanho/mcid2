...
cases_by_cond_pl <- epi_data %>%
  ...
  geom_text(mapping = aes(label = cases),
            size = 8 / cm(1), vjust = -1) +
  labs(x = "Air cleaner", y = "Number of respiratory cases", fill = "Air cleaner", title = "c") +
  scale_fill_manual(values = wes_palette("Royal1")[1:2]) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.2)), breaks = scales::pretty_breaks(4)) +
  theme_custom() +
