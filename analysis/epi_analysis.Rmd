...
cases_by_week_pl <- epi_data %>%
  ...
  geom_bar_pattern(mapping = aes(pattern = class),
                   stat = "identity", color = "black", pattern_fill = "white", pattern_angle = 45,
                   pattern_density = 0.25, pattern_spacing = 0.05, width = .5) +
  geom_text(data = epi_data %>% group_by(aircleaner) %>% summarize(cases = sum(new_cases)) %>% ungroup(),
            mapping = aes(label = cases),
            size = 8 / cm(1), vjust = -1) +
  labs(x = "Air cleaner", y = "Number of cases", fill = "Air cleaner", pattern = "Class", title = "a") +
...
avoid_infections_pl <- avoid_infections  %>%
  ...
  labs(x = "Estimated number of infections", y = "Scenario", fill = "CrI", title = "b") +
  theme_custom() +
